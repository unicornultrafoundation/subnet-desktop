...
export async function findChecksum(checksumURL: string, executableName: string): Promise<string> {
  ...
  const desiredChecksums = allChecksums
    .split(/\r?\n/)
  ...
  throw new Error(
    `Matched ${desiredChecksums.length} hits, not exactly 1, for ${executableName} in [${allChecksums}]`
  )
}
...
export async function writeDependencyVersions(
  path: string,
...
export interface Dependency {
  ...
  rcompareVersions(
    version1: string | AlpineLimaISOVersion,
...
export function getOctokit(personalAccessToken?: string): Octokit {
  ...
  if (!personalAccessToken) {
    throw new Error(
      'Please set GITHUB_TOKEN to a PAT to check versions of github-based dependencies.'
    )
  }
  ...
  function makeLimitHandler(
    type: string,
    maxRetries: number
  ): NonNullable<ThrottlingOptions['onSecondaryRateLimit']> {
...
export async function getPublishedReleaseTagNames(
  owner: string,
  repo: string,
  githubToken?: string
) {
