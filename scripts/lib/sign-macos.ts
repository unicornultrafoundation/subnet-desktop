...
export async function sign(workDir: string): Promise<string[]> {
  ...
  if (certFingerprint.length < 1) {
    throw new Error(
      `CSC_FINGERPRINT environment variable not set; required to pick signing certificate.`
    )
  }
  ...
  await Promise.all(
    signingConfig.remove.map(async (relpath) => {
  ...
  for await (const filePath of findFilesToSign(appDir)) {
    ...
    const entitlementsOverride = signingConfig.entitlements.overrides.find((e) =>
      e.paths.includes(relPath)
    )
      ...
      for (const constraintType of constraintTypes) {
        ...
        if (constraint) {
          const constraintsFile = path.join(
            plistsDir,
            `${fileHash}-constraint-${constraintType}.plist`
          )
  ...
  await spawnFile('codesign', ['--verify', '--deep', '--strict', '--verbose=2', appDir], {
    stdio: 'inherit'
  })
  ...
  await Promise.all(
    [dmgRenamedFile, zipFile].map((f) => {
      return spawnFile('codesign', ['--sign', certFingerprint, '--timestamp', f], {
        stdio: 'inherit'
      })
    })
  )
