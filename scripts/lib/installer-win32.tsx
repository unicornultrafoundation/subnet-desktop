 ...
 */
export default async function buildInstaller(
  workDir: string,
  appDir: string,
  outFile = ''
): Promise<string> {
  ...
  const template = await fs.promises.readFile(
    path.join(process.cwd(), 'build', 'wix', 'main.wxs'),
    'utf-8'
  )
  ...
  await Promise.all(
    inputs.map((input) =>
      simpleSpawn(path.join(wixDir, 'candle.exe'), [
        '-arch',
        'x64',
        `-dappDir=${appDir}`,
        ...
        '-nologo',
        '-out',
        path.join(workDir, `${path.basename(input, '.wxs')}.wixobj`),
        '-pedantic',
        '-wx',
        '-ext',
  ...
  await simpleSpawn(
    ...
    [
      ...
      '-wx',
      '-cc',
      path.join(process.cwd(), 'dist', 'wix-cache'),
      '-reusecab',
      '-loc',
      path.join(path.join(process.cwd(), 'build', 'wix', 'string-overrides.wxl')),
...
async function writeUpdateConfig(appDir: string) {
  ...
  const electronBuilderConfig = yaml.parse(
    await fs.promises.readFile(path.join(appDir, 'electron-builder.yml'), 'utf-8')
  )
