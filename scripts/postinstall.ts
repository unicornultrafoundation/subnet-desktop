...
const unixDependencies: Dependency[] = [new Lima(), new Qemu(), new AlpineLimaISO()]
...
async function downloadDependencies(items: DependencyWithContext[]): Promise<void> {
  ...
  const forwardDependencies = Object.fromEntries(
    items.map((item) => [specialize(item), [] as string[]] as const)
  )
  ...
  for (const item of items) {
    ...
    for (const dependency of dependencies) {
      if (dependency in reverseDependencies) {
        ...
        throw new Error(
          `Dependency ${item.dependency.name} depends on unknown dependency ${dependency}`
        )
      }
    }
  }
  ...
  for (const item of items.filter(
    (d) => (d.dependency.dependencies?.(d.context) ?? []).length === 0
  )) {
  ...
  if (all.size > done.size) {
    const remaining = Array.from(all)
      .filter((d) => !done.has(d))
      .sort()
...
async function runScripts(): Promise<void> {
  ...
  if (platform === 'linux' || platform === 'darwin') {
    ...
    dependencies.push(
      ...vmDependencies.map((dependency) => ({ dependency, context: vmDownloadContext }))
    )
...
async function buildDownloadContextFor(
  rawPlatform: DependencyPlatform,
  ...
  await Promise.all(
    dirsToCreate.map((d) => fs.promises.mkdir(downloadContext[d], { recursive: true }))
  )
...
;(async () => {
  ...
  try {
    await runScripts()
    await simpleSpawn('node', ['node_modules/electron-builder/out/cli/cli.js', 'install-app-deps'])
    exitCode = 0
  } catch (e: any) {
