...
import { getProductionVersion } from '../utils/version'
...
export function createSettings(deploymentProfiles: DeploymentProfileType): Settings {
  ...
  if (
    !Object.keys(deploymentProfiles.defaults).length &&
    !Object.keys(deploymentProfiles.locked).length
  ) {
    _isFirstRun = true
  }
...
function finishConfiguringSettings(
  cfg: Settings,
  deploymentProfiles: DeploymentProfileType
): Settings {
 ...
 */
export function determineLockedFields(
  lockedProfileSettings: LockedSettingsType
): LockedSettingsType {
  ...
  return Object.fromEntries(
    Object.entries(lockedProfileSettings).map(([k, v]) => {
 ...
 * in AppData\Local. The only file it needs to preserve is `AppData\Roaming\subnet-desktop\settings.json`.
...
function migrateSettingsLocationOnWindows(): boolean {
  ...
  const oldConfigPath = join(appData, 'subnet-desktop', 'settings.json')
...
export const updateTable: Record<number, (settings: any, locked: boolean) => void> = {
  1: (settings) => {
    _.unset(settings, 'kubernetes.subnetMode')
  },
  2: (_) => {
  ...
  4: (settings) => {
    ...
    const replacements: ReplacementDirective[] = [
      ...
      {
        oldPath: 'kubernetes.experimental.socketVMNet',
        newPath: 'experimental.virtualMachine.socketVMNet'
      },
  ...
  5: (settings) => {
    const replacements: ReplacementDirective[] = [
      ...
      {
        oldPath: 'virtualMachine.experimental.socketVMNet',
        newPath: 'experimental.virtualMachine.socketVMNet'
      }
    ]
  ...
  6: (settings) => {
    ...
    if (_.hasIn(settings, 'extensions')) {
      const withTags = Object.entries(settings.extensions ?? {})
        .filter(([, v]) => v)
        .map(([k]) => k)
  ...
  7: (_settings) => {},
  8: (settings) => {
    // subnet Desktop 1.10: move .extensions to .application.extensions.installed
    const replacements: ReplacementDirective[] = [
 ...
 */
export function migrateSpecifiedSettingsToCurrentVersion(
  settings: Record<string, any>,
  locked: boolean,
  targetVersion: number = CURRENT_SETTINGS_VERSION
): RecursivePartial<Settings> {
...
// Setting IDs
export const SETTING = { PL_SUBNET_VALUE: 'subnet' }
