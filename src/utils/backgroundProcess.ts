...
export default class BackgroundProcess {
  ...
  protected async restart() {
    ...
    if (this.process) {
      if (this.isRunning()) {
        console.debug(
          `Restarting ${this.name} (pid ${this.process.pid}): ignoring restart, already alive`
        )
    ...
    process.on('exit', (status, signal) => {
      if ([0, null].includes(status) && ['SIGTERM', null].includes(signal)) {
        ...
        console.log(
          `Background process ${this.name} (pid ${process.pid}) exited with status ${status} signal ${signal}`
        )
      }
      if (!Object.is(process, this.process)) {
        console.log(
          `Not current ${this.name} process (pid ${process.pid}, want ${this.process?.pid}); nothing to be done.`
        )
      ...
      this.shouldRun()
        .then((result) => {
          if (result) {
            ...
            console.debug(
              `Background process ${this.name} will restart (process ${process.pid} exited)`
            )
          }
        })
        .catch(console.error)
