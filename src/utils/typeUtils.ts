...
type UpperSnakeCaseInner<T extends string> = T extends ''
  ? never
  : T extends UpperAlpha
    ? `_${T}`
    : T extends Alpha<T>
      ? Uppercase<T>
      : T extends `${infer C}${infer U}`
        ? `${UpperSnakeCaseInner<C>}${UpperSnakeCaseInner<U>}`
        : never
 ...
 */
export type UpperSnakeCase<T extends string | symbol | number> = T extends symbol | number
  ? never
  : T extends Alpha<T>
    ? Uppercase<T>
    : T extends `${infer C}${infer U}`
      ? `${Uppercase<C>}${UpperSnakeCaseInner<U>}`
      : T
...
export type RecursiveKeys<T> =
  Record<string, unknown> extends T
    ...
    : T extends readonly unknown[]
      ? RecursiveKeys<T[number]>
      : T extends Record<string, unknown>
        ? (keyof T & string) | RecursiveKeysInner<T, keyof T & string>
        : never

type RecursiveKeysInner<T, K extends string> = K extends keyof T
  ? `${K}.${RecursiveKeys<T[K]>}`
  : never
...
export type RecursiveTypes<T extends Record<string, any>> =
  Record<string, unknown> extends T
    ? never
    : {
        [P in RecursiveKeys<T>]: P extends keyof T
          ? T[P]
          : P extends `${infer K}.${infer R}`
            ? K extends keyof T
              ? T[K] extends Record<string, unknown>
                ? R extends keyof RecursiveTypes<T[K]>
                  ? RecursiveTypes<T[K]>[R]
                  : never
                : never
              : never
            : never
