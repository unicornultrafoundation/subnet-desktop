...
export { ChildProcess, exec, execFile, spawn } from 'child_process'
export type { CommonOptions, SpawnOptions } from 'child_process'
...
interface SpawnOptionsWithStdioLog<Stdio extends IOType | Log> extends SpawnOptionsLog {
  stdio: Stdio
}
...
export async function spawnFile(command: string): Promise<Record<string, never>>
...
export async function spawnFile(
  command: string,
  options: SpawnOptionsWithStdioTuple<StdioNull | StdioPipe, StdioPipe, StdioPipe> &
    SpawnOptionsEncoding
): Promise<{ stdout: string; stderr: string }>
export async function spawnFile(
  command: string,
  options: SpawnOptionsWithStdioTuple<StdioNull | StdioPipe, StdioPipe, StdioNullLog> &
    SpawnOptionsEncoding
): Promise<{ stdout: string }>
export async function spawnFile(
  command: string,
  options: SpawnOptionsWithStdioTuple<StdioNull | StdioPipe, StdioNullLog, StdioPipe> &
    SpawnOptionsEncoding
): Promise<{ stderr: string }>
export async function spawnFile(
  command: string,
  options: SpawnOptionsWithStdioTuple<StdioNull | StdioPipe, StdioNullLog, StdioNullLog> &
    SpawnOptionsEncoding
): Promise<Record<string, never>>

export async function spawnFile(command: string, args: string[]): Promise<Record<string, never>>
export async function spawnFile(
  command: string,
  args: string[],
  options: SpawnOptionsWithStdioLog<'ignore' | 'inherit' | Log> & SpawnOptionsEncoding
  ...
  promises.push(
    new Promise<void>((resolve, reject) => {
      child.on('exit', (code, signal) => {
        ...
        reject(
          new SpawnError([command].concat(finalArgs), {
            code,
            signal,
            ...result
          })
